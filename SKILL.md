...
**Read `references/detection-guidance.md` before rewriting anything, and check the final
rewrite against the three countable thresholds in `references/numeric-checks.md`.** It holds the false-positive list, which is what a tell only looks like, and the signs of human writing to leave alone. Every pattern above can fire on prose a person wrote on purpose, so those carve-outs are the difference between a rewrite that helps and one that flattens the piece.
